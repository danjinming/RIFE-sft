...
def train(model):
    dataset = VimeoDatasetSFT('train')
    train_data = DataLoader(dataset, batch_size=args.batch_size, num_workers=2, pin_memory=True, drop_last=True, shuffle=True)
    
    dataset_val = VimeoDatasetSFT('validation')
    val_data = DataLoader(dataset_val, batch_size=args.batch_size, num_workers=2, pin_memory=True, shuffle=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--batch_size', default=2, type=int, help='minibatch size')
    args = parser.parse_args()
    model = Model()
    result = model.load_model('weights/flownet.426.pkl')
